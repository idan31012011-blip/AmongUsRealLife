...
export default function RoleRevealScreen() {
  const { state } = useGame();
  const { t } = useLanguage();
  const { myRole, gameCode, isDoctor, isEngineer, isAnalyst, isManager } = state;
  ...
  return (
    ...
    >
      ...
      {stage === 'revealed' && isAnalyst && (
        <div className="doctor-reveal-badge analyst-reveal-badge">
          <span className="doctor-reveal-icon">📊</span>
          <div>
            <div className="doctor-reveal-title">{t('analystNotification')}</div>
            <div className="doctor-reveal-desc">{t('analystNotificationDesc')}</div>
          </div>
        </div>
      )}
